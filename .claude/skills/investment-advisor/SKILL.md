...
② 写入 Obsidian vault（可浏览，双链互联）
  如 vault_enabled=true → obsidian-cli 写入 vault
  否则 → 写入 D:\Agent\git\investment-advisor\notes\<日期-主题>.md

③ 同步到 KB 文档目录（可 RAG 检索）
  同一份 .md 同时复制到 D:\Agent\git\knowledge-base\docs\
```

> 这样一份笔记两个用途：Obsidian 里能翻能链，RAG 里能搜能查。互不割裂。
...
当 docs/ 目录攒了一批新笔记后，提醒用户重建向量索引：

```bash
cd D:\Agent\git\knowledge-base && python ingest.py
```

> 不用每记一条笔记就跑——攒个十几二十条再跑一次就行。
